...
def _observable_matrices_for_repr(repr_mode: str, n_qubits: int) -> list[np.ndarray]:
    ...
    if repr_mode == "random":        
        ...
        n_selected = args.pullback_sample_top_n
        selected_idx = rng.choice(len(observables), size=n_selected, replace=False)
        return [observables[int(i)] for i in selected_idx]
...
def concept_accuracy(
    ...
    for concept_name in concept_map:
        ...
        if args.pullback_use_uniform_set:
            pullback_uniform_indices = top_indices
        else:
...
def _extract_pullback_concept_features(
    ...
    use_uniform_set = args.pullback_use_uniform_set
    ...
    if use_uniform_set:
        if uniform_top_indices is None:
            ...
            top_indices = uniform_top_indices
    else:
    ...
    if top_indices.ndim == 1:
        top_indices = np.tile(top_indices[None, :], (len(x_input), 1))
...
def _build_global_explanation_bank(
    ...
    return {
        ...
        "pullback_uniform_indices": uniform_top_indices,
    }
...
def global_explanations(
    ...
    if concept_bank is None:
        concept_bank = _build_global_explanation_bank(model, random_seed, train_loader)
    ...
    for feed_dict in tqdm(test_loader, unit="batch", leave=False):
        ...
        if model.repr_mode == "pullback":
            if feature_csv_path.exists():
                ...
                if getattr(model, "_pullback_observable_indices", None) is not None:
                    selected_names = list(model._pullback_observable_names)
                    h_test = batch_df[selected_names].to_numpy(dtype=np.float32, copy=True)
                else:
...
if __name__ == "__main__":
    ...
    args.repr = "random"  # Options: "statevector", "pullback", "local_x", "local_y", "local_z", "local_zz"
    args.poison_x = 0.0
    args.poison_y = 0.0
    ...
    quality_csv = Path.cwd() / "results" / "mnist_tq" / args.task / f"{args.repr}_concept_bank_seed_quality.csv"
    ...
    overall_csv_path = Path.cwd() / "results" / "poison" / "global_explanations" / "overall_metrics.csv"
