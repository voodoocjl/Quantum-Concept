...
def _build_xyz_zz_observables(n_qubits: int):
    """Build fixed observable family [X_i, Y_i, Z_i, Z_i Z_j] with stable ordering."""
...
def pullback_feature_matrix(
    ...
    """Compute pullback coefficients for a dataset and optionally keep only top indices."""
    coeffs = []
...
def concept_accuracy(
    ...
    # Fit a concept classifier and test accuracy for each concept using pure-state features.
    results_data = []
...
def global_explanations(
    ...
    # Fit concept classifiers in pure-state feature space.
    results_data = []
...
def adversarial_robustness(
    ...
    for attack_prop in [0, 0.05, 0.1, 0.2, 0.5, 0.7, 1]:
        ...
        for X_test, y_test in tqdm(test_loader, unit="batch", leave=False):
            ...
            car_preds = [car.predict(h_test) for car in car_classifiers]
            cav_preds = [
                cav.concept_importance_chainrule(
                    X_test,
                    H_test,
                    y_test.to(device),
                    10,
                    model.representation_to_output,
                    state_to_observable_features_torch,
                )
