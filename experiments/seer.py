...
from models.seer import SEERClassifier, SEERVQCClassifier
...
def train_model(
    ...
    # model = SEERVQCClassifier(model_name)
    try:
        model.load_state_dict(torch.load(model_dir / f"{model_name}.pt"), strict=False)
    except:
        pass
    model.fit(device, train_loader, test_loader, model_dir, n_epoch=50, patience=50)
