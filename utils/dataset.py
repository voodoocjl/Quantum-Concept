...
from torchquantum.dataset import MNIST as TQMNIST
...
def MNISTDataLoaders(args, task):
    ...
    n_train_samples = getattr(args, "n_train_samples", None)
    n_valid_samples = getattr(args, "n_valid_samples", None)
    n_test_samples = getattr(args, "n_test_samples", None)
    same_n_samples_each_class = getattr(args, "same_n_samples_each_class", False)
    ...
    for split in dataset:
        if split == 'train':
            ...
            # Keep validation and test ordering deterministic for evaluation.
...
def generate_mnist_concept_dataset(
    ...
    *legacy_args,
    ...
    Args:
        ...
        data_loader: existing dataloader that yields MNIST-like batches (preferred)
    ...
    for batch in data_loader:
        if isinstance(batch, dict):
            ...
            raise ValueError("Unsupported MNIST batch format; expected dict or tuple/list")

        x_batches.append(images.detach().cpu())
        y_batches.append(labels.detach().cpu())

    if len(x_batches) == 0:
        raise ValueError("Received an empty data_loader while building concept dataset")

    all_images = torch.cat(x_batches, dim=0)
    all_labels = torch.cat(y_batches, dim=0).to(dtype=torch.long)

    concept_set = {int(c) for c in concept_classes}
    concept_tensor = torch.tensor(sorted(concept_set), dtype=torch.long)
    positive_mask = torch.isin(all_labels, concept_tensor)

    positive_idx = torch.nonzero(positive_mask).flatten()
    negative_idx = torch.nonzero(~positive_mask).flatten()

    if len(positive_idx) < subset_size or len(negative_idx) < subset_size:
        raise ValueError(
            f"Not enough examples in loader for subset_size={subset_size}: "
            f"positives={len(positive_idx)}, negatives={len(negative_idx)}"
        )

    torch.manual_seed(random_seed)
    selected_positive_idx = positive_idx[torch.randperm(len(positive_idx))[:subset_size]]
    selected_negative_idx = negative_idx[torch.randperm(len(negative_idx))[:subset_size]]

    positive_images = all_images[selected_positive_idx].numpy()
    negative_images = all_images[selected_negative_idx].numpy()
    X = np.concatenate((positive_images, negative_images), 0)
